...
%%%%%
%% EDIT : immediately below, look in the data to see how much of the data is actually dropped from the analysis
%%%%%
...
%%%%%
%% EDIT: Need to work on the below
%%%%%
...
\stepcounter{footnote}\footnotetext{
	...
	This usually involves simulating independent variables, specifying an effect size, simulating a dependent variable given the independent variables and the effect size, and then testing if the effect size can 
}
...
%%%%%
%% EDIT: Add a final paragraph here about the main story
%%%%%
...
%%%%%
%% EDIT: Find the ROCL issue Glenn mentions
%%%%%

\addtocounter{footnote}{-1}
\stepcounter{footnote}\footnotetext{
	There are normative reasons for wanting an accurate classification process.
}
...
\begin{table}[ht!] %	\centering %	\captionsetup{justification=centering}
	\centering
	\caption{Expected Welfare Surplus Difference, $\text{HN}_{240}$ Approach}
	\label{tb:HN3_win05_pop}
	\begin{adjustbox}{}
	\pgfplotstabletypeset[
		col sep=comma,
		every head row/.style={
...
\begin{table}[ht!] %	\centering %	\captionsetup{justification=centering}
	...
	\pgfplotstabletypeset[
		...
		]{ch4/tables/HNG_3-win_05_full-table.csv} % path/to/file
	\end{adjustbox}
\end{table}
...
In Tables \ref{tb:HN1_default_pop} through \ref{tb:HN13_win05_pop} we see the average of the predictions for the hypothetical population for the Default approach and the $\text{HN}_\text{C}$ approach for $C \in \lbrace 80, 240, 400, 560, 1040\rbrace$.
...
Even a more moderate increase in the number of lottery pairs to increase the probability of correctly classifying $\mathit{RDU_{Prelec}}$ subjects, to 400 for intance, can create methodological concerns.
...
For a hypothetical population parameterized by the point estimates of real subject data, the probability of correctly classifying $\mathit{RDU_{Prelec}}$ subjects is shown to be less than 40\%.
...
Nonetheless, I conclude that utilizing the proposed \enquote{Default} approach of classification or increasing the sample size by several hundred lottery pairs per subject would result in more accurate subjective welfare estimates.

These two approaches, increasing the sample size and disregarding classification altogether, are not the only options available.
There exists the possibility of alternative experimental design and/or econometric procedures.
Econometrically, one can imagine a Baysian approach in which small groups of subjects are grouped together based on observable characteristics, age, sex, education, etc., and then pooled estimations from these subgroups being used as priors to inform the individual level estimates of the members of the groups.
Different classification processes could be used as well
