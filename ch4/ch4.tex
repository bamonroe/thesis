...
\stepcounter{footnote}\footnotetext{
	...
	This usually involves simulating independent variables, specifying an effect size, simulating a dependent variable given the independent variables and the effect size, and then testing how frequently the effect size can recovered from multiple repetitions of the simulated data.
}
...
Note that these figures show welfare surplus estimates, \textit{not} the WSD metric, which is shown in the tables below.
...
The probability of correctly classifying EUT subjects with the $\text{HN}_{80}$ is much greater than that of $\mathit{RDU_{Prelec}}$ subjects at 88\%, and rapidly approaches the 95\% limit.
The $\mathit{RDU_{Prelec}}$ subjects however, are not correctly classified 95\% of the time for any of the repetitions, as seen in the second row second column of each plot, and only reaches 80\% correct classification probability with greater than 400 lottery pairs per subject.
...
These figures provide some distributional information, as well as show the raw welfare surplus estimates for these two approaches whereas Tables \ref{tb:HN1_default_pop} and \ref{tb:HN1_win05_pop} provide metrics for the average and expected WSD, and classification probabilities for the same approaches.
...
Additionally, non or semi-parameter estimation techniques may fare better in terms of classification accuracy.
...
In terms of instrument design, there are more than 1 septillion ($10^{24} < 2^{80}$) possible choice patterns in the HN lottery instrument.
...
The task of modifying the experimental design and econometric procedure while guarding against concerns proposed by experimental methodology is Herculean, and this chapter, unfortunately, can not provide much guidance with respect to this task beyond increasing the sample size.
