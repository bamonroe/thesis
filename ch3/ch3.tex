...
\subsection{Notation and Estimation} \label{ssec:Notation}
...
\addtocounter{footnote}{-1}
\stepcounter{footnote}\footnotetext{
	This process could be used to help with the econometric limitations of the pure RP model as those subjects who violate FOSD can be picked up by an alternative model which permitted such violations. 
...
While it is theoretically possible to have a mixture model with greater than two underlying stochastic specifications, in reality this is computationally demanding and thus the mixture model presented in (\ref{eq:PT_Mix}) is often only utilized with one or two mixtures.
...
\addtocounter{footnote}{-1}
\stepcounter{footnote}\footnotetext{
	As discussed in Chapter 2, the primary concern of economic analysis is to provide useful information about the welfare implications of policy or institutional rules.
...
Equations (\ref{eq:Pnt}) through (\ref{eq:LLnT}) are computationally impossible to estimate directly due to the general \enquote{inability of computers to perform integration} in a closed-form \parencite[2]{Train2002}.
...
Equation (\ref{eq:SPnt}) needs some explanation.
...
\stepcounter{footnote}\footnotetext{
	See the remainder of \textcite[Chapter~9]{Train2002} for an in-depth discussion and derivation of why Halton sequences are widely viewed as being superior to other pseudo-random number generators for the purposes of simulating estimators.
}
...
With this simulated unconditional probability, we can obtain the simulated unconditional likelihood by substituting equation (\ref{eq:SPnt}) for equation (\ref{eq:Pnt}) in equation (\ref{eq:LnT}):
\begin{equation}
	\label{eq:SLnT}
	\mathit{SL}_{nT}(\theta) = \prod_t^T \left[ \mathlarger{\sum}_h^H \frac{ P_{nt}(\beta^h) }{H} \right]
\end{equation}

Equation (\ref{eq:SLnT}) is limited in terms of identifying $\theta$ because, as indicated by the $n$ subscript, this metric is defined for a single agent.
...
This is an easy extension of equation (\ref{eq:SLnT}), which is logged for numerical stability:
\begin{equation}
	\label{eq:SLLNT}
	\mathit{SLL}_{NT}(\theta) = \sum_{n=1}^N \left( \sum_t^T \left[ \ln\!\left( \sum_h^H \frac{ P_{nt}(\beta^h) }{H} \right) \right] \right)
\end{equation}

\noindent We call equation (\ref{eq:SLLNT}) the unconditional simulated log-likelihood function, or just the simulated log-likelihood function (SLL).
...
The characteristics of simulated estimators are reviewed in depth by \textcite[Chapter~10]{Train2002}, and the critical insight is that the estimator $\hat{\theta}$ derived from equation (\ref{eq:SLLNT}) approaches the estimator from equation (\ref{eq:LLnT}) with a sufficiently large, $H$, number of draws from the distribution governed by $\theta$.
...
\stepcounter{footnote}\footnotetext{
	...
	\enquote{Saliency} requires that the rules and mechanisms that map an agent's actions to outcomes be understood and actionable by the agent.
	\enquote{Domiance} requires that the value of the outcome resulting from one action is sufficiently different to the value of an outcome resulting from another action to the extent that this difference \textit{dominates} the cost of performing such an action.
...
Throughout this chapter, we will refer to the metric in equation (\ref{eq:WST}) as the \enquote{welfare surplus} metric.
...
\noindent In the metric defined in equation (\ref{eq:WET}), the {\CE}'s of the options chosen by the agent across all tasks $T$ are summed, and then divided by the {\CE}'s of the options that would have provided the greatest utility across all the tasks.
...
Throughout this chapter, we will refer to the metric in equation (\ref{eq:WET}) as the \enquote{welfare efficiency} metric.
...
The welfare surplus and welfare efficiency metrics from equations (\ref{eq:WST}) and (\ref{eq:WET}) can be used in place of equation (\ref{eq:MTBn}) in equation (\ref{eq:EMt}) to gather useful information for a given choice pattern and $\theta$ vector:
\begin{align}
	E( \Delta W_T | \theta) &= \int \Delta W_T(\beta) f(\beta | \theta) d \beta \label{eq:EWST}\\
	E( \% W_T | \theta) &= \int \% W_T(\beta) f(\beta | \theta) d \beta \label{eq:EWET}
\end{align}

Given equation (\ref{eq:NMB}), we can denote the expected welfare surplus and the expected welfare efficiency obtained by agents who have committed $e \in [0,T]$ errors by making choices $y_t \times T$ as follows:
\begin{align}
...
The same limitation mentioned about MSL concerning a computer's inability to perform closed-form integration in general applies to equations (\ref{eq:EMt}), (\ref{eq:PE}), and (\ref{eq:EWST}) through (\ref{eq:EWET}).
...
Throughout the following discussion, we will refer to a choice pattern's likelihood of being \textit{observed}, by which we mean the choice pattern's simulated likelihood as calculated in equation (\ref{eq:SLnT}).
...
\addtocounter{footnote}{-1}
\stepcounter{footnote}\footnotetext{
	Not only are we assuming that agents do not have random preferences, we're also assuming that an agent's preferences are static across choices generally.
...
The metrics described in equations (\ref{eq:SLnT}) and (\ref{eq:MTBn}) through (\ref{eq:EPWTe}) rely on a given choice pattern, $y_t \times T$.
In the HL-MPL there are a total of $2^{10}=1024$ choice patterns that can be described.
To begin the discussion of the welfare implications of stochastic choice models, we calculate the values for equations (\ref{eq:SLnT}), and (\ref{eq:MTBn}) through (\ref{eq:EPWTe}) for all $\mathit{TT} =1024$ choice patterns and all $e \in[0,T]$ for the given $\theta$, with $H=2.5 \times 10^6$.{\footnotemark} 

\addtocounter{footnote}{-3}
\stepcounter{footnote}\footnotetext{
	This is done for convenience; adding correlation among the marginal distributions would require the definition of a covariance matrix.
...
First, we evaluate equation (\ref{eq:PnT}), the likelihood that agent $n$ would produce this choice pattern, utilizing equation (\ref{eq:RE}) to calculate choice probabilities for the individual tasks:
\begin{align}
	\label{eq:example_PnT}
	\begin{split}
...
This information allows us to evaluate equation (\ref{eq:MTBn}), the frequency of choice errors in a given choice pattern, utilizing equation (\ref{eq:Itb}):

\begin{align}
	\label{eq:example_MTBn}
	\begin{split}
...
This result allows us to calculate equation (\ref{eq:NMB}) for values of $e \in [ 0, T ]$ which indicates if there have been $e$ number of errors in this choice pattern:

\begin{align}
	\label{eq:example_NMB}
	\begin{split}
...
\noindent Next we can calculate the two welfare metrics from equations equations (\ref{eq:WST}) and (\ref{eq:WET}), indicating welfare surplus and welfare efficiency respectively.
First we calculate the {\CE} of option $A$ and option $B$ for all $T = 10$ tasks using equation (\ref{eq:CEcalc}). 
...
With the {\CE}'s calculated, we can substitute them in to equations (\ref{eq:WST}) and (\ref{eq:WET}).
For equation equation (\ref{eq:WST}), we take the sum of column $6$ in Table \ref{tb:example_CE}:

\begin{equation}
	\label{eq:example_WST}
	\Delta W_{nT} = \sum_{t=1}^T \left( {\CE}_{nyt} - {\CE}_{n1t}^Z \right) = 8.92
\end{equation}

\noindent and for equation (\ref{eq:WET}), we take the sum of column $4$ and divide it by the sum of column $7$:

\begin{equation}
	\label{eq:example_WET}
	\%W_{nT} = \frac{\displaystyle\sum_{t=1}^{T} {\CE}_{nyt} }{\displaystyle\sum_{t=1}^{T} {\CE}_{n1t}} = \frac{21.37}{21.74} = .983
\end{equation}

\noindent Finally, we multiply the welfare metrics derived in equations (\ref{eq:example_WST}) and (\ref{eq:example_WET}) by the indicator functions derived for $e \in [0,T]$ in equation (\ref{eq:NMB}):

\begin{align}
	\label{eq:example_NMBWST}
	\begin{split}
...
\noindent The indicator functions in equation (\ref{eq:example_NMB}) are mutually exclusive, therefore the product of the indicator functions and the welfare metrics will be $0$ for all but one value of $e$, and equal to the metrics for the remaining $e$, in this example, for $e = 1$. 

%Thus, the derivation of the results in equations (\ref{eq:example_PnT}) through (\ref{eq:example_WET}) constitutes the core of the computational exercise that results in population level expectations.
...
To arrive at the population level metrics, we take the average of each metric defined in equations (\ref{eq:example_PnT}) through (\ref{eq:example_NMBWET}) across all $H$ simulated agents for each choice pattern.
...
This resulting dataset, however, is too large to be displayed in full, so for now we restrict attention to the 10 choice patterns most likely to be observed, and discuss the metrics calculated in equations (\ref{eq:EMt}), (\ref{eq:EWST}), (\ref{eq:EWET}), and (\ref{eq:PE}) with $e = (0,1)$.
...
\begin{table}[ht]
	...
	\centering
	\caption{HL-MPL Welfare and Error Expectations for Top Ten Most Likely Choice Patterns, EUT}
	\label{tb:TopTenEUT}
	\begin{adjustbox}{width=1\textwidth}
	\pgfplotstabletypeset[
		col sep=comma,
...
This is shown by the values in column $P_E(e=0)$, which reference equation (\ref{eq:PE}), all being less than $0.50$.
...
Before beginning the analysis of this mixture population, we can extend the metrics utilized in equations (\ref{eq:SLnT}) and (\ref{eq:MTBn}) through (\ref{eq:EPWTe}) to be defined for mixed populations.
...
Once again, we will employ an $H = 2.5 \times 10^6$ and calculate the values for equations (\ref{eq:SLnT}) and (\ref{eq:MTBn}) through (\ref{eq:EPWTe}) for all $\mathit{TT} =1024$ choice patterns and all $e \in[0,T]$ for the RDU subpopulation.
...
%The welfare metric described in equation (\ref{eq:EWET}) provides a useful benchmark to describe the difference between the welfare evaluation done on the individual level versus welfare evaluation done on the sample level.
...
For instance, for each $y \times T$ choice pattern, we can weigh the expected welfare efficiency resulting from equation (\ref{eq:EWET}) by the simulated likelihood resulting from equation (\ref{eq:SLnT}) and then sum across all TT choice patterns to retrieve the sample expected welfare efficiency:
...
We can substitute any  $y \times T$ statistic from derived from equations (\ref{eq:EMt}), (\ref{eq:PE}), (\ref{eq:EWET}), and (\ref{eq:EPWTe}) in place of $\%W_T$ in equations (\ref{eq:EPWTT}) and (\ref{eq:VPWTT}) to describe these statistics on a population by population basis.
...
Given these limitations, instead we will examine the relationship of the parameters making up the stochastic specification, i.e. the elements of $\theta$, with the associated results of equations (\ref{eq:EPWTT}) and (\ref{eq:VPWTT}) visually and with the use of locally weighted polynomial regression (LOESS) as originally developed by Cleveland, Grosse, Shyu, Chambers \& Hastie (1992).
...
For each population type, we generate 500,000 unique population parameter sets, $\theta_i$, the elements of which are assumed to be uncorrelated, and solve equations (\ref{eq:EPWTT}) and (\ref{eq:VPWTT}) for the statistics derived in equations (\ref{eq:EMt}), (\ref{eq:PE}) and (\ref{eq:EWET}) with $e=0$ and with each equation solved with $H=10,000$.
