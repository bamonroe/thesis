...
\begin{equation}
	...
	\omega(p_i) = \frac{p_i^\gamma}{\biggl(\sum\limits_{k=1}^{I} p_k^\gamma\biggr)^{ \frac{1}{\gamma} } }
\end{equation}
...
\stepcounter{footnote}\footnotetext{
	...
	All deterministic theories of utility require the dominant option to be chosen over the dominated option.
}
...
%%% EDIT
% Find BW1997 assumptions that they found unsavory and put in a footnote
...
\stepcounter{footnote}\footnotetext{
	...
	As will be discussed later, the RP model fails to satisfy this restriction.
}
\stepcounter{footnote}\footnotetext{
...
\stepcounter{footnote}\footnotetext{ 
	E.g, trembles could apply differently to FOSD pairs and non-FOSD pairs, and RP models could assume flexible distributions of the agent's preferences such as the Logit-Normal or Gamma distributions.
}
\stepcounter{footnote}\footnotetext{ 
...
%%% EDIT
% Above with the "more risk averse than" - explain why it allows for interpersonal comparisons of risk-aversion
...
When evaluating option $j$, a standard preference relation is drawn from a distribution of preference relations that ranks each of the $J$ alternatives, including option $j$, according to this preference relation:
\begin{equation}
...
But, with RPPO models, when evaluating another option $k$, such that $k \neq j$, another, potentially different, preference relation is drawn and each of the $J$ alternatives, including both $j$ and $k$, are ranked according to this preference relation:
\begin{equation}
...
\addtocounter{footnote}{-1}
\stepcounter{footnote}\footnotetext{
	The reader can consult \textcite{Cubitt2001} for a novel methodology on atheoretic representations of decision problems, as well as the examples indicated.
}
