...
Continued investigation of this class of models has largely ceased since these and additional results by \textcite{Camerer1994}.\footnote{
	\textcite{Sopher2000} provide the only experimental evidence I'm aware of subsequent to the results of \textcite{Camerer1994} that are favorable to the \enquote{stochastic options} theory.
	This study however does not employ the same kind of statistical tests as either \textcite{Hey1995}.
}
...
This \enquote{{\CE}} approach is similar to the \enquote{money-metric utility} function by \textcite{Samuelson1974} employed to calculate welfare surplus.
...
On the other hand, \textcite{Braga2009}, in a similar experiment, find that while most anomalies are eliminated with market exposure, others arise as rounds progress and that there may be a \enquote{just enough} amount of market exposure to induce conformity to EUT.
...
However, it isn't clear which choices should be corrected given an RP model and a set of choices which don't conform to EUT.
In the stand-alone RP model, each choice represents the welfare maximal option for the preference relation drawn for that task.
Changing choices in an aggregate set of choices which deviate from EUT implies that some choices will 

even if aggregated choices result in the strict reduction in the agent's stock of assets.
